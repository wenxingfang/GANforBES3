...
if __name__ == '__main__':
    ...
    if convert2pb:
        ...
        print('input 0 is :', Model.input[0].name)
        print('input 1 is :', Model.input[1].name)
    ...
    if parse_args.exact_model == 'True':
        ...
        for line in f_info:
            ...
            M_dtheta = float(M_dtheta.split('=')[-1])
            M_dphi   = float(M_dphi  .split('=')[-1])/10
            P_dz     = float(P_dz    .split('=')[-1])
            P_dphi   = float(P_dphi  .split('=')[-1])
            P_z      = float(P_z     .split('=')[-1])/100
    ...
    if removeZ:
        generator_inputs = [noise, sampled_info[:,0:5]]
    ...
    actual_info[:,5] = actual_info[:,5]*100
    #print ('actual_info\n:',actual_info[0:10])
