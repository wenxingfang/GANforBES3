...
def get_parser():
    ...
    parser.add_argument('--use_MEA_gen_model', action='store',type=ast.literal_eval,
                        default=False,
                        help='use_MEA_gen_model')
    parser.add_argument('--gen_MEA_model_in', action='store',type=str,
                        default='',
                        help='gen_MEA_model_in')
...
if __name__ == '__main__':
    ...
    if parse_args.use_MEA_gen_model:
        gen_MEA_model = load_model(parse_args.gen_MEA_model_in, custom_objects={'tf': tf})
        images_MEA = gen_MEA_model.predict(generator_inputs, verbose=True)
        hf.create_dataset('Barrel_Hit_MEA', data=images_MEA[0])
    ### using combined model to check discriminator and regression part  ############
    if parse_args.comb_model_in !='':
        comb_model = load_model(parse_args.comb_model_in, custom_objects={'tf': tf,'MyDense2D':MyDense2D(output_dim1=10, output_dim2=10), 'single_layer_energy':single_layer_energy})
    ...
    if parse_args.check_dis_real =='True' and parse_args.dis_model_in !='':
        dis_model = load_model(parse_args.dis_model_in, custom_objects={'tf': tf,'MyDense2D':MyDense2D(output_dim1=10, output_dim2=10), 'single_layer_energy':single_layer_energy})
        d = h5py.File(parse_args.real_data, 'r')
