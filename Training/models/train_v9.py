...
###################################
    ...
    for epoch in range(nb_epochs):
        ...
        for index in range(nb_batches):
            ...
            tried = 0
            real_batch_loss = 0
            real_test_loss = discriminator.test_on_batch( [image_batch,  info_batch],  disc_outputs_real,  loss_weights )
            while True:
                real_batch_loss = discriminator.train_on_batch( [image_batch,  info_batch],  disc_outputs_real,  loss_weights )
                print('real_test_loss=',real_test_loss,',real_batch_loss=',real_batch_loss)
                if real_batch_loss < real_test_loss: break
                tried = tried + 1
                if tried > Max_iter : break

            #print('real_batch_loss=',real_batch_loss)
            fake_batch_loss = 0
            fake_test_loss = discriminator.test_on_batch( [generated_images[0], generated_images[1] ],  disc_outputs_fake, loss_weights )
            tried = 0
            while True:
                fake_batch_loss = discriminator.train_on_batch( [generated_images[0], generated_images[1] ],  disc_outputs_fake, loss_weights )
                print('fake_test_loss=',fake_test_loss,',fake_batch_loss=',fake_batch_loss)
                if fake_batch_loss < fake_test_loss: break
                tried = tried + 1
                if tried > Max_iter : break
            ...
            while True:
                ...
                print('gen_test_loss=',gen_test_loss,',gen_train_loss=',gen_train_loss)
