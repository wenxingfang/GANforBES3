...
if __name__ == '__main__':
    ...
    from ops import (minibatch_discriminator, minibatch_output_shape, Dense3D,
                     calculate_energy, scale, inpainting_attention, sparsity_level)
    ...
    mc_info_v1[:,1] = (mc_info_v1[:,1])/180  #dtheta
    mc_info_v1[:,2] = (mc_info_v1[:,2])/10   #dphi
    mc_info_v1[:,3] = (mc_info_v1[:,3])/100  #Z
    ...
    p = concatenate([features, depth_info, input_Info, energies, sparsity])
    '''
        well_too_big,
    '''
    ...
    img_layer  = build_generator_2D_v1(h, 12, 16)
    print('img_layer shape:',img_layer.shape)
    
    flat = Flatten()(img_layer)
    #depth_layer = Dense(1, name='depth_output')(flat)
    depth1 = Dense(100,activation='relu')(h)
    depth2 = Dense(10, activation='relu')(depth1)
    depth_layer = Dense(1, name='depth_output')(depth2)
...
###################################
    ...
    combined_outputs = [discriminator(generator(generator_inputs)), reg_model(generator(generator_inputs)[0:2]) ]
    print('h31')
    ...
    for epoch in range(nb_epochs):
        ...
        mc_info_v1_mom      = shuffle(mc_info_v1[:,0]    , random_state=1)
        mc_info_v1_dphi     = shuffle(mc_info_v1[:,2]    , random_state=2)
        mc_info_v1_dtheta_z = shuffle(mc_info_v1[:,[1,3]], random_state=3)
        mc_info_v1_dtheta_z = np.insert(mc_info_v1_dtheta_z,0,mc_info_v1_mom ,axis=1)
        mc_info_v1_dtheta_z = np.insert(mc_info_v1_dtheta_z,2,mc_info_v1_dphi,axis=1)
        print('mc_info_v1_dtheta_z:',mc_info_v1_dtheta_z.shape) 
        mc_info_v2_mom      = shuffle(mc_info_v1[:,0]    , random_state=4)
        mc_info_v2_dphi     = shuffle(mc_info_v1[:,2]    , random_state=5)
        mc_info_v2_dtheta_z = shuffle(mc_info_v1[:,[1,3]], random_state=6)
        mc_info_v2_dtheta_z = np.insert(mc_info_v2_dtheta_z,0,mc_info_v2_mom ,axis=1)
        mc_info_v2_dtheta_z = np.insert(mc_info_v2_dtheta_z,2,mc_info_v2_dphi,axis=1)
        ...
        for index in range(nb_batches):
            ...
            info_batch_v1 =  mc_info_v1_dtheta_z[index * batch_size:(index + 1) * batch_size]
            '''
            ...
            info_batch_v2 =  mc_info_v2_dtheta_z[index * batch_size:(index + 1) * batch_size]
            max_try = 10
            tmp_try =0
            while True:
                tmp_try = tmp_try + 1
                ...
                combined_inputs  = [noise, info_batch_v2]
                ...
                combined_outputs = [np.ones(batch_size), info_batch_v1]
