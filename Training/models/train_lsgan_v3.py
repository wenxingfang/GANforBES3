...
buffer for some exact generator iter
Add EMA
"""
...
if __name__ == '__main__':
    ...
    from ops import (minibatch_discriminator, minibatch_output_shape, Dense3D,
                     calculate_energy, scale, inpainting_attention, sparsity_level, get_mc_info, single_layer_relative_diff_too_big, calculate_e3x3, calculate_e5x5, single_layer_energy, single_layer_relative_diff, mini_batch_mean, mini_batch_std , is_High_Z, single_layer_relative, latMom, secondMom, calculate_e1x1, ExponentialMovingAverage)
...
#        n_mc_info = 5
    ...
    print('h3')
    EMAer = ExponentialMovingAverage(generator) # 在模型compile之后执行
...
###################################
    ...
    for epoch in range(nb_epochs):
        ...
        for index in range(nb_batches):
            ...
            if buffer_first:
                ...
                if buffer_image.shape[0] < buffer_size:
                    ...
                    b_i = b_i%(int(buffer_image.shape[0]/batch_size))
                    bindex = list(range(b_i*batch_size, (b_i+1)*batch_size))
        ...
        tmp_gen_out_name_ema  = (parse_args.gen_out).replace('.h5','_EMA_epoch%d.h5'%epoch)
        ...
        if (epoch+1)%1==0 and True :
            ...
            generator.save    (tmp_gen_out_name_ema)
