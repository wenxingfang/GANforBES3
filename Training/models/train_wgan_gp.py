...
if __name__ == '__main__':
    ...
    from ops import (minibatch_discriminator, minibatch_output_shape, Dense3D,
                     calculate_energy, scale, inpainting_attention, sparsity_level, get_mc_info, single_layer_relative_diff_too_big, calculate_e3x3, calculate_e5x5, single_layer_energy, single_layer_relative_diff, mini_batch_mean, mini_batch_std ,normalize ,MyDense2D, single_layer_relative)

    from architectures import build_generator_2D, build_discriminator_2D, build_generator_2D_v1, build_discriminator_2D_v2, RandomWeightedAverage, RandomWeightedAverage1, wasserstein_loss, gradient_penalty_loss 
    ...
    real_la_Flatten_1 = la_Flatten_1(real_la_Dropout_3)
    ...
    real_con2         = concatenate([real_sparsity, real_mom, real_en, real_e3x3, real_e5x5, real_en_diff, real_en_ratio, real_too_big])
    ...
    fake_la_Flatten_1 = la_Flatten_1(fake_la_Dropout_3)
    ...
    fake_con2         = concatenate([fake_sparsity, fake_mom, fake_en, fake_e3x3, fake_e5x5, fake_en_diff, fake_en_ratio, fake_too_big])
    ...
    aveg_la_Flatten_1 = la_Flatten_1(aveg_la_Dropout_3)
    ...
    aveg_con2         = concatenate([aveg_sparsity, aveg_mom, aveg_en, aveg_e3x3, aveg_e5x5, aveg_en_diff, aveg_en_ratio, aveg_too_big])
...
###################################
    ...
    #combined_outputs = [discriminator([generator(generator_inputs)[0], generator(generator_inputs)[1], generator(generator_inputs)[0], generator(generator_inputs)[1] ]), reg_model([ generator(generator_inputs)[0], generator(generator_inputs)[2] ] ) ]
    ...
    for epoch in range(nb_epochs):
        ...
        for index in range(nb_batches):
            ...
            disc_outputs =[-1*np.ones(batch_size), np.ones(batch_size), np.zeros(batch_size)]
            ...
            if index == (nb_batches-1):
                print('disc_loss:\n',disc_batch_loss)
                #disc_loss = discriminator.predict_on_batch([image_batch, info_batch, generated_images[0], generated_images[1]])
            ...
            while True:
                ...
                #combined_outputs =[[-1*np.ones(batch_size),np.zeros(batch_size),np.zeros(batch_size)], info_batch_v2[:,0:3] ]
                combined_outputs =[-1*np.ones(batch_size), info_batch_v2[:,0:3] ]
