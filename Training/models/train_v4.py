...
if __name__ == '__main__':
    ...
    from ops import (minibatch_discriminator, minibatch_output_shape, Dense3D,
                     calculate_energy, scale, inpainting_attention, sparsity_level, get_mc_info, calculate_energy_diff)
    ...
    mc_info[:,5] = (mc_info[:,5])/100  #Z
    mc_info_v1[:,2] = (mc_info_v1[:,2])/10   #dphi
    mc_info_v1[:,5] = (mc_info_v1[:,5])/100  #Z
...
###################################
    ...
    for epoch in range(nb_epochs):
        ...
        mc_info_v1_M_dtheta = shuffle(mc_info_v1[:,1:2]    , random_state=2)
        mc_info_v1_M_dphi   = shuffle(mc_info_v1[:,2:3]    , random_state=3)
        mc_info_v1_P_dz     = shuffle(mc_info_v1[:,3:4]    , random_state=4)
        mc_info_v1_P_dphi   = shuffle(mc_info_v1[:,4:5]    , random_state=5)
        mc_info_v1_P_z      = shuffle(mc_info_v1[:,5:6]    , random_state=6)
        ...
        mc_info_v2_mom      = shuffle(mc_info_v1[:,0:1]    , random_state=11)
        mc_info_v2_M_dtheta = shuffle(mc_info_v1[:,1:2]    , random_state=12)
        mc_info_v2_M_dphi   = shuffle(mc_info_v1[:,2:3]    , random_state=13)
        mc_info_v2_P_dz     = shuffle(mc_info_v1[:,3:4]    , random_state=14)
        mc_info_v2_P_dphi   = shuffle(mc_info_v1[:,4:5]    , random_state=15)
        mc_info_v2_P_z      = shuffle(mc_info_v1[:,5:6]    , random_state=16)
        ...
        for index in range(nb_batches):
            ...
            while True:
                ...
                combined_outputs = [np.ones(batch_size), info_batch_v2]
