...
if __name__ == '__main__':
    ...
    from architectures import build_generator_2D, build_discriminator_2D, build_generator_2D_v1, build_generator_2D_v2, build_discriminator_2D_v2, RandomWeightedAverage, RandomWeightedAverage1, wasserstein_loss, gradient_penalty_loss 
...
###################################
    ...
    combined_losses = [ wasserstein_loss, 'mae']
    ...
    for epoch in range(nb_epochs):
        ...
        for index in range(nb_batches):
            ...
            ########################
            noise            = np.random.normal(0, 1, (batch_size, latent_size))
            combined_inputs  = np.concatenate ((noise, info_batch_v1), axis=-1)
            combined_outputs =[-1*np.ones(batch_size), info_batch_v1[:,0:3] ]
            ...
            #generator_inputs = np.concatenate ((noise, info_batch), axis=-1) ## try to use same info to produce fake image
