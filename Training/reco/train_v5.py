...
def get_parser():
    ...
    parser.add_argument('--removeZ', action='store', type=ast.literal_eval, default=False,
...
if __name__ == '__main__':
    ...
    from ops import (minibatch_discriminator, minibatch_output_shape, Dense3D,
                     calculate_energy, scale, inpainting_attention, calculate_e3x3, calculate_e5x5, calculate_e1x1, single_layer_relative_diff)
    ...
    rel_diff_e3x3 = Lambda(single_layer_relative_diff)([energies, energies_3x3])
    print('rel_diff_e3x3:', rel_diff_e3x3.shape)
    rel_diff_e5x5 = Lambda(single_layer_relative_diff)([energies, energies_5x5])
    print('rel_diff_e5x5:', rel_diff_e5x5.shape)
    rel_diff_e1x1 = Lambda(single_layer_relative_diff)([energies, energies_1x1])
    print('rel_diff_e1x1:', rel_diff_e1x1.shape)

    p_en = concatenate([energies, energies_3x3, energies_5x5, energies_1x1, rel_diff_e3x3, rel_diff_e5x5, rel_diff_e1x1])
    p_e_1 = Dense(10, name='p_e_1')(p_en)
    p_e_2 = Dense(10, activation='relu', name='p_e_2')(p_e_1)
    p_e_3 = Dense(1 , name='p_e_3')(p_e_2)
    
    #p = concatenate([features, energies, energies_3x3, energies_5x5, energies_1x1, rel_diff_e3x3, rel_diff_e5x5, rel_diff_e1x1, Z_info])
    p = concatenate([features, p_e_3, Z_info])
    reg = Dense(5, name='reg_output')(p)
    regression_outputs = reg
    #regression_losses = 'mae'
    regression_losses = 'mse'

    #regression = Model(calorimeter, regression_outputs)
    regression = Model([calorimeter, Z_info], regression_outputs)
    ...
    for epoch in range(nb_epochs):
        ...
        for ib in range(len(Batch)):
            ...
            for index in range(ibatch):
                ...
                info_batch  = mc_info[index * batch_size:(index + 1) * batch_size,0:5]
                Z_info_batch= mc_info[index * batch_size:(index + 1) * batch_size,5:6]
